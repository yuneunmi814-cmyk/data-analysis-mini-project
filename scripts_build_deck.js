...
function divider(num,t,sub){let s=p.addSlide(); s.background={color:NAVY};
  ...
  s.addText(sub,{x:0.92,y:5.4,w:11.5,h:0.5,fontSize:16,color:"9FB2CC",fontFace:F,margin:0});}
...
const methods=[["t-test","2호선 vs 그 외 역당 이용객 2.15배 — t=4.83, p<.001, Cohen's d=1.08(큰 효과)"],
...
evid({k:"근거 02 · 호선별 비교",h:"혼잡은 2호선에 집중 — 유의할 뿐 아니라 효과도 크다",img:"근거2_호선Top8.png",
  cards:[["η²=0.28","ANOVA 大효과(28% 설명)",RED],["d=1.08","t-test 큰 효과",NAVY]],bul:["t-test: 2호선 vs 그외 2.15배 (t=4.83, p<.001)","ANOVA F=14.8, p<.001 · Tukey 2호선>6개선","→ 2호선 우선 투자 근거"],key:"t-test·ANOVA 모두 유의 + 실질 효과(η²=.28, d=1.08) 큼",src:"서울 열린데이터광장(2026); 팀 분석(t-test·ANOVA·Tukey)"});
